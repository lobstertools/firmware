...
OneButton button(ONE_BUTTON_PIN, true,
                 true); // Pin, active low, internal pull-up
...
portMUX_TYPE timerMux =
    portMUX_INITIALIZER_UNLOCKED; // Critical section for tick counter
...
// --- WiFi Retry Logic ---
int g_wifiRetries = 0;
volatile bool g_triggerProvisioning =
    false; // Flag to trigger blocking mode from loop
...
int startSession(unsigned long duration, unsigned long penalty,
                 TriggerStrategy strategy, unsigned long *delays, bool hide);
int startTestMode();
void stopTestMode();
void abortSession(const char *source);
void triggerLock(const char *source);
void enterLockedState(
    const char *source); // Shared by Auto-Countdown and Trigger
...
void sendJsonError(AsyncWebServerRequest *request, int code,
...
void handleArm(AsyncWebServerRequest *request, uint8_t *data, size_t len,
...
void handleUpdateWifi(AsyncWebServerRequest *request, uint8_t *data, size_t len,
                      size_t index, size_t total);
...
#define PROV_ENABLE_PAYBACK_TIME_CHAR_UUID                                     \
...
void startBLEProvisioning() {
  ...
  auto createChar = [&](const char *uuid) {
    BLECharacteristic *p =
        pService->createCharacteristic(uuid, BLECharacteristic::PROPERTY_WRITE);
...
void startMDNS() {
  ...
  snprintf(uniqueHostname, sizeof(uniqueHostname), "lobster-lock-%02X%02X%02X",
           mac[3], mac[4], mac[5]);
...
void armFailsafeTimer() {
  ...
  uint64_t timeout_us =
      (uint64_t)g_systemConfig.failsafeMaxLockSeconds * 1000000ULL;
  esp_timer_start_once(failsafeTimer, timeout_us);

  char timeStr[64];
  formatSeconds(g_systemConfig.failsafeMaxLockSeconds, timeStr,
                sizeof(timeStr));
...
void WiFiEvent(WiFiEvent_t event) {
  ...
  case ARDUINO_EVENT_WIFI_STA_GOT_IP:
    ...
    char logBuf[64];
    snprintf(logBuf, sizeof(logBuf), "WiFi: Connected. IP: %s",
  ...
  case ARDUINO_EVENT_WIFI_STA_DISCONNECTED:
    ...
    if (g_wifiRetries >= g_systemConfig.wifiMaxRetries) {
      logMessage(
          "WiFi: Max retries exceeded. Falling back to BLE Provisioning...");
      ...
      snprintf(logBuf, sizeof(logBuf),
               "WiFi: Disconnected (Attempt %d/%d). Retrying...",
...
void setup() {
  ...
  const esp_timer_create_args_t failsafe_timer_args = {
      .callback = &failsafe_timer_callback, .name = "failsafe_wdt"};
  ...
  unsigned long longPressMs =
      (unsigned long)g_systemConfig.longPressSeconds * 1000;
  if (longPressMs < 1000) {
    longPressMs = 1000;
  }
...
void checkSystemHealth() {
  ...
  if (!isnan(currentTemp) && currentTemp > MAX_SAFE_TEMP_C) {
    ...
    char logBuf[100];
    snprintf(logBuf, sizeof(logBuf),
             "CRITICAL: Overheating detected (%.1f C)! Emergency Stop.",
             currentTemp);
    logMessage(logBuf);
...
void abortSession(const char *source) {
  ...
  if (currentState == LOCKED) {
    ...
    if (enablePaybackTime) { // 3. Add payback
      ...
      snprintf(logBuf, sizeof(logBuf), " %-20s : +%u s", "Payback Added",
               paybackTimeSeconds);
      logMessage(logBuf);
    ...
    snprintf(logBuf, sizeof(logBuf),
             "%s: Abort ignored, device not in abortable state.", source);
    logMessage(logBuf);
    return;
  }
}
...
void completeSession() {
  ...
  if (previousState == LOCKED) {
    ...
    if (paybackAccumulated > 0) {
      logMessage(
          " > Valid session complete. Clearing accumulated payback debt.");
    ...
    snprintf(logBuf, sizeof(logBuf), " %-20s : %u", "New Streak",
             sessionStreakCount);
    logMessage(logBuf);
    snprintf(logBuf, sizeof(logBuf), " %-20s : %u", "Total Completed",
             completedSessions);
    logMessage(logBuf);
...
void resetToReady(bool generateNewCode) {
  ...
  if (generateNewCode) {
    ...
    logMessage(logBuf);
    snprintf(logBuf, sizeof(logBuf), " %-20s : %s...", "Code Snippet",
             codeSnippet);
    logMessage(logBuf);
    snprintf(logBuf, sizeof(logBuf), " %-20s : %s", "Checksum",
             rewardHistory[0].checksum);
    logMessage(logBuf);
  } else {
...
void setupWebServer() {
  ...
  // API: Get the main device status (dynamic data, polled frequently). (GET
  // /status)
  ...
  server.on(
      "/update-wifi", HTTP_POST, [](AsyncWebServerRequest *request) {}, NULL,
      handleUpdateWifi);
...
void handleRoot(AsyncWebServerRequest *request) {
  String html =
      "<html><head><title>" + String(DEVICE_NAME) + "</title></head><body>";
  html += "<h1>" + String(DEVICE_NAME) + " API</h1>";
  html += "<ul>";
  html +=
      "<li><b>GET /status</b> - Real-time metrics (lock timer, state).</li>";
...
void handleKeepAlive(AsyncWebServerRequest *request) {
  ...
  if (xSemaphoreTakeRecursive(stateMutex, (TickType_t)pdMS_TO_TICKS(100)) ==
      pdTRUE) {
    ...
    if (currentState == LOCKED || currentState == TESTING) {
      ...
      if (g_currentKeepAliveStrikes > 0) {
        char logBuf[100];
        snprintf(logBuf, sizeof(logBuf),
                 "Keep-Alive Watchdog: Signal received. Resetting %d strikes.",
                 g_currentKeepAliveStrikes);
        logMessage(logBuf);
      }
...
void handleArm(AsyncWebServerRequest *request, uint8_t *data, size_t len,
  ...
  if (error) {
    char logBuf[100];
    snprintf(logBuf, sizeof(logBuf), "Failed to parse /arm JSON: %s",
             error.c_str());
    logMessage(logBuf);
  ...
  bool newHideTimer =
      (*doc)["hideTimer"] | false; // Default to false if not present
  ...
  if (xSemaphoreTakeRecursive(stateMutex, (TickType_t)pdMS_TO_TICKS(1000)) ==
      pdTRUE) {

    logMessage(LOG_SEP_MAJOR);
    logMessage("API REQUEST: /arm");

    int result = startSession(durationSeconds, penaltySeconds, requestedStrat,
                              tempDelays, newHideTimer);
    ...
    if (result == 200) {
      ...
      (*responseDoc)["triggerStrategy"] =
          (requestedStrat == STRAT_BUTTON_TRIGGER) ? "buttonTrigger"
                                                   : "autoCountdown";
...
void handleAbort(AsyncWebServerRequest *request) {
  ...
  if (xSemaphoreTakeRecursive(stateMutex, (TickType_t)pdMS_TO_TICKS(1000)) ==
    ...
    (*doc)["status"] =
        (currentState == ABORTED)
            ? "aborted"
            : (currentState == COMPLETED ? "completed" : "ready");
    serializeJson(*doc, responseJson);
...
void handleReward(AsyncWebServerRequest *request) {
  ...
  if (xSemaphoreTakeRecursive(stateMutex, (TickType_t)pdMS_TO_TICKS(1000)) ==
      pdTRUE) {
    if (currentState == LOCKED || currentState == ABORTED ||
        currentState == ARMED) {
      xSemaphoreGiveRecursive(stateMutex);
...
void handleStatus(AsyncWebServerRequest *request) {
  ...
  // Quick Lock & Copy
  if (xSemaphoreTakeRecursive(stateMutex, (TickType_t)pdMS_TO_TICKS(200)) ==
      pdTRUE) {
  ...
  if (snapshot.state == ARMED) {
    doc["triggerStrategy"] = (snapshot.strategy == STRAT_BUTTON_TRIGGER)
                                 ? "buttonTrigger"
                                 : "autoCountdown";
...
void handleDetails(AsyncWebServerRequest *request) {
  ...
  if (xSemaphoreTakeRecursive(stateMutex, (TickType_t)pdMS_TO_TICKS(1000)) ==
    ...
    esp_efuse_mac_get_default(mac);
    char uniqueHostname[20];
    snprintf(uniqueHostname, sizeof(uniqueHostname),
             "lobster-lock-%02X%02X%02X", mac[3], mac[4], mac[5]);
...
void handleUpdateWifi(AsyncWebServerRequest *request, uint8_t *data, size_t len,
  ...
  if (error) {
    char logBuf[100];
    snprintf(logBuf, sizeof(logBuf), "Failed to parse /update-wifi JSON: %s",
             error.c_str());
    logMessage(logBuf);
    sendJsonError(request, 400, "Invalid JSON body.");
    return;
  ...
  if (xSemaphoreTakeRecursive(stateMutex, (TickType_t)pdMS_TO_TICKS(1000)) ==
    ...
    if (currentState != READY) {
      ...
      sendJsonError(request, 409,
                    "Device must be in READY state to update Wi-Fi.");
      return;
    }
...
void handleFactoryReset(AsyncWebServerRequest *request) {
  ...
  if (xSemaphoreTakeRecursive(stateMutex, (TickType_t)pdMS_TO_TICKS(1000)) ==
    ...
    logMessage(
        "API: /factory-reset received. Erasing credentials and session data.");
  ...
  (*doc)["message"] =
      "Device credentials and state erased. Rebooting into provisioning mode.";
  String response;
  serializeJson(*doc, response);
  request->send(200, "application/json", response);
...
void startTimersForState(SessionState state) {
  ...
  if (state == LOCKED || state == ABORTED || state == TESTING) {
    ...
    updateWatchdogTimeout(
        DEFAULT_WDT_TIMEOUT); // Loose loop check (network delays)
  }
...
bool checkKeepAliveWatchdog() {
  ...
  if (calculatedStrikes > g_currentKeepAliveStrikes) {
    ...
    if (g_currentKeepAliveStrikes >= g_systemConfig.keepAliveMaxStrikes) {
      snprintf(logBuf, sizeof(logBuf),
               "Keep-Alive Watchdog: Strike %d/%d! ABORTING.",
               g_currentKeepAliveStrikes, g_systemConfig.keepAliveMaxStrikes);
      logMessage(logBuf);
      abortSession("Watchdog Strikeout");
      return true; // Signal that we aborted
    } else {
      snprintf(logBuf, sizeof(logBuf),
               "Keep-Alive Watchdog: Missed check. Strike %d/%d",
               g_currentKeepAliveStrikes, g_systemConfig.keepAliveMaxStrikes);
      logMessage(logBuf);
    }
  }
...
bool loadState() {
  ...
  currentStrategy = (TriggerStrategy)sessionState.getUChar(
      "strategy", (uint8_t)STRAT_AUTO_COUNTDOWN);
  ...
  sessionState.getBytes("delays", channelDelaysRemaining,
                        sizeof(channelDelaysRemaining));
...
void saveState(bool force) {
  ...
  char logBuf[100];
  snprintf(logBuf, sizeof(logBuf), "Saving state to NVS: %s",
           stateToString(currentState));
  logMessage(logBuf);
  ...
  sessionState.putBytes("delays", channelDelaysRemaining,
                        sizeof(channelDelaysRemaining));
...
void logMessage(const char *message) {
  ...
  if (xSemaphoreTakeRecursive(stateMutex, (TickType_t)pdMS_TO_TICKS(100)) ==
    ...
    if (nextHead != serialQueueTail) {
      // Buffer not full
      snprintf(serialLogQueue[serialQueueHead], MAX_LOG_ENTRY_LENGTH, "%s",
               message);
...
void processLogQueue() {
  ...
  while (maxLinesToProcess > 0) {
    ...
    // cycle
    if (xSemaphoreTakeRecursive(stateMutex, (TickType_t)pdMS_TO_TICKS(5)) ==
        pdTRUE) {
...
void calculateChecksum(const char *code, char *outString) {
  ...
  snprintf(outString, REWARD_CHECKSUM_LENGTH, "%s-%02d", getNatoWord(alphaChar),
           rollingVal);
}
  ...
  while (collision) {
    ...
    for (int i = 1; i < REWARD_HISTORY_SIZE; i++) {
      ...
      if (strlen(rewardHistory[i].checksum) > 0) {
        if (strncmp(checksumBuffer, rewardHistory[i].checksum,
                    REWARD_CHECKSUM_LENGTH) == 0) {
...
void setLedPattern(SessionState state) {
  char logBuf[50];
  snprintf(logBuf, sizeof(logBuf), "Setting LED pattern for: %s",
           stateToString(state));
  logMessage(logBuf);
